 ...
 *		-	<GenerateDirectoryNames()> must be called before <InputDirectoryNameOf()> will work.  Currently
 *			it is called by <NaturalDocs::Menu->LoadAndUpdate()>.
 *
 *	Architecture: Internal Overview
 *		-	<Load()> first parses the command line, gathering all the settings and checking for errors.
 *			All <NaturalDocs::Builder> packages must be registered before this is called because it needs
 *			their command line options.  <NaturalDocs::Project->ReparseEverything()> and
 *			<NaturalDocs::Project->RebuildEverything()> are called right away if -r or -ro are used.
 *
 *		-	Output directories are *not* named at this point.  See <Named Directories>.
 *
 *		-	The previous settings from the last time Natural Docs was run are loaded and compared to
 *			the current settings.  <NaturalDocs::Project->ReparseEverything()> and
 *			<NaturalDocs::Project->RebuildEverything()> are called if there are any differences that warrant it.
 *
 *		-	It then waits for <GenerateDirectoryNames()> to be called by <NaturalDocs::Menu>.  The
 *			reason for this is that the previous directory names are stored as hints in the menu
 *			file, for reasons explained in <Named Directories>.  Once that happens all the unnamed
 *			directories have names generated for them so everything is named.  The package is completely set up.
 *
 *		-	The input directories are stored in an array instead of a hash because the order they were
 *			declared in matters.  If two people use multiple input directories on separate computers
 *			without sharing a menu file, they should at least get consistent directory names by declaring
 ...
 *	Since they don't necessarily extend from the same root anymore, they can't share an output directory
 *	without the risk of file name conflicts.  There was an early attempt at giving them actual names,
 *	but now they're just numbered from 1.
 *
 *	Directory names aren't generated right away.  It waits for <Menu.txt> to load because that holds the
 *	obfuscated names from the last run.  <NaturalDocs::Menu> then calls <GenerateDirectoryNames()> and
 *	passes those along as hints. <GenerateDirectoryNames()> then applies them to any matches and generates
 *	new ones for any remaining.  This is done so that output page locations can remain consistent when built
 *	on multiple computers, so long as the menu file is shared.  I tend to think the menu file is the most
 *	likely configuration file to be shared.
 *
 ...
 *	The primary reason, though there may be others, is file purging.  If an input directory is removed,
 *	all the output files that were generated from anything in it need to be removed.  To find out what
 *	the output file name was for a removed source file, it needs to be able to split it from it's
 *	original input directory and know what that directory was named.  If this didn't happen those
 *	output files would be  orphaned, as was the case prior to 1.32.
 */
class PHP_Natural_Docs_Settings
{
	/******************************************************************************
	 *	Group: Variables
	 */
	 ...
	 */
	static protected $inputDirectories;
	 ...
	 */
	static protected $inputDirectoryNames;

	/**
	 * 	array: imageDirectories
	 * 	An array of image directories.
	 */
	static protected $imageDirectories;

	/**
	 * 	array: imageDirectoryNames
	 * 	An array of the image directory names.  Each name corresponds to the directory of the
	 * 	same index in <imageDirectories>.
	 */
	static protected $imageDirectoryNames;
	 ...
	 */
	static protected $relativeImageDirectories;

	/**
	 * 	array: excludedInputDirectories
	 * 	An array of input directories to exclude.
	 */
	static protected $excludedInputDirectories;

	/**
	 * 	array: removedInputDirectories
	 * 	An array of input directories that were once in the command line but are no longer.
	 */
	static protected $removedInputDirectories;

	/**
	 * 	array: removedInputDirectoryNames
	 *	An array of the removed input directories' names.  Each name corresponds to the directory
	 *	of the same index in <removedInputDirectories>.
	 */
	static protected $removedInputDirectoryNames;

	/**
	 * 	array: removedImageDirectories
	 * 	An array of image directories that were once in the command line but are no longer.
	 */
	static protected $removedImageDirectories;

	/**
	 * 	array: removedImageDirectoryNames
	 * 	An array of the removed image directories' names.  Each name corresponds to the directory
	 * 	of the same index in <removedImageDirectories>.
	 */
	static protected $removedImageDirectoryNames;

	/**
	 * 	var: projectDirectory
	 * 	The project directory.
	 */
	static protected $projectDirectory;

	/**
	 * 	array: buildTargets
	 * 	An array of <NaturalDocs::Settings::BuildTarget>s.
	 */
	static protected $buildTargets;

	/**
	 * 	var: documentedOnly
	 * 	Whether undocumented code aspects should be included in the output.
	 */
	static protected $documentedOnly;

	/**
	 * 	int: tabLength
	 * 	The number of spaces in tabs.
	 */
	static protected $tabLength;

	/**
	 * 	bool: noAutoGroup
	 * 	Whether auto-grouping is turned off.
	 */
	static protected $noAutoGroup;

	/**
	 * 	bool: onlyFileTitles
	 * 	Whether source files should always use the file name as the title.
	 */
	static protected $onlyFileTitles;

	/**
	 * 	bool: isQuiet
	 * 	Whether the script should be run in quiet mode or not.
	 */
	static protected $isQuiet;

	/**
	 * 	bool: rebuildData
	 * 	Whether most data files should be ignored and rebuilt.
	 */
	static protected $rebuildData;
	 ...
	 * 	Will exit if the options are invalid or the syntax reference was requested.
	 */
	 ...
	 *
	 * 	Generates names for each of the input and image directories, which can later
	 * 	be retrieved with <InputDirectoryNameOf()> and <ImageDirectoryNameOf()>.
	 *
	 * 	Parameters:
	 * 		inputHints	-	A hashref of suggested input directory names, where the keys are
	 * 						the directories and the values are the names. These take precedence
	 * 						over anything generated.  You should include names for directories
	 * 						that are no longer in the command line.  This parameter may be undef.
	 * 		imageHints	-	Same as inputHints, only for the image directories.
	 */
	static public function generateDirectoryNames(&$inputHints,&$imageHints)
	{
		/** CONVERT_PERL_PHP
		my %usedInputNames;
		my %usedImageNames;
		...
		{
			#	First, we have to convert all non-numeric names to numbers, since they may come
			#	from a pre-1.32 menu file.  We do it here instead of in NaturalDocs::Menu to
			#	keep the naming scheme centralized.
			...
			if ($hasNonNumeric){
				...
				if (scalar @names == 1){
					...
					#	We sort the list first because we want the end result to be predictable.
					#	This conversion could be happening on many machines, and they may not all
					#	specify the input directories in the same order.  They need to all come up
	 ...
	 */
	static public function inputDirectories()
	{
		return self::$inputDirectories;
	}
	 ...
	 *	<GenerateDirectoryNames()> must be called once before this function is available.
	 *
	 *	If a name for a removed input directory is available, it will be returned as well.
	 */
	static public function inputDirectoryNameOf($directory)
	{
		/** CONVERT_PERL_PHP
		for (my $i = 0; $i < scalar @inputDirectories; $i++){
	 ...
	 */
	static public function splitFromInputDirectory($file)
	{
		/** CONVERT_PERL_PHP
		foreach my $directory (@inputDirectories, @removedInputDirectories){
			if (NaturalDocs::File->IsSubPathOf($directory, $file)){
				return array($directory, NaturalDocs::File->MakeRelativePath($directory, $file));
			}
		}
		*/
		return array();
	 ...
	 */
	static public function imageDirectories()
	{
		return self::$imageDirectories;
	}
	 ...
	 */
	static public function imageDirectoryNameOf($directory)
	{
		/** CONVERT_PERL_PHP
		for (my $i = 0; $i < scalar @imageDirectories; $i++){
	 ...
	 */
	static public function splitFromImageDirectory($file)
	{
		/** CONVERT_PERL_PHP
	 ...
	 */
	static public function relativeImageDirectories()
	{
		//	FIXME: method is same as variable, this won't work in PHP
		return self::$relativeImageDirectories;
	}
	 ...
	 */
	static public function excludedInputDirectories()
	{
		//	FIXME: method is same as variable, this won't work in PHP
		return self::$excludedInputDirectories;
	}
	 ...
	 */
	static public buildTargets()
	{
		//	FIXME: method is same as variable, this won't work in PHP
		return self::$buildTargets;
	}
	 ...
	 */
	static public function outputDirectoryOf($object)
	{
		/** CONVERT_PERL_PHP
	...
	{
		//	FIXME: function name is same as variable
		return self::$styles;
	}
	 ...
	 */
	static public function projectDirectory()
	{
		//	FIXME: function name is same as variable
	 ...
	 */
	static public function projectDataDirectory()
	{
	 ...
	 */
	static public function styleDirectory()
	{
	 ...
	 */
	static public function javaScriptDirectory()
	{
	 ...
	 */
	static public documentedOnly()
	{
		return self::$documentedOnly;
	}
	 ...
	 */
	static public function tabLength()
	{
		return self::$tabLength;
	}
	 ...
	 */
	static public function noAutoGroup()
	{
		return self::$noAutoGroup;
	}
	 ...
	 */
	static public function onlyFileTitles()
	{
		//	FIXME: won't work, variable is same as function name
		return self::$onlyFileTitles;
	}
	 ...
	 */
	static public function isQuiet()
	{
		return self::$isQuiet;
	}
	 ...
	 */
	static public function highlightCode()
	{
		//	FIXME: won't work, variable name is same as function name
		return self::$highlightCode;
	}
	 ...
	 */
	static public function highlightAnonymous()
	{
		//	FIXME: won't work, variable name is same as function name
		return self::$highlightAnonymous;
	}
	 ...
	 */
	static public function textAppVersion()
	{
		return '1.52';
	}
	 ...
	 *
	 * 	Parses and validates the command line.  Will cause the script to exit if the
	 * 	options ask for the syntax reference or are invalid.
	 */
	static public function parseCommandLine()
	{
		...
		while ($index < scalar @ARGV){
			...
			if (substr($arg, 0, 1) eq '-'){
				...
				if ($option eq '-i'){
					...
					}elseif ($option eq '-nag'){
						$noAutoGroup = 1;
					}elseif ($option eq '-?' || $option eq '-h'){
						$self->PrintSyntax();
						exit;
					}else{
		...
		foreach my $outputString (@outputStrings){
			...
			if (!defined $directory){
				...
				}elseif(exists $outputDirectories{$directory}){
		...
		foreach my $imageString (@imageStrings){
			//	FIXME: this regexp breaks the code comments :'(
			if ($imageString =~ /^ *\* /){
		...
		if (defined $highlightString){
			...
			}elseif($highlightString eq 'code'){
				$highlightCode = 1;
				$highlightAnonymous = undef;
			}elseif ($highlightString eq 'all'){
				$highlightCode = 1;
